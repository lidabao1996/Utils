...
public class ExcelUtil {
	 ...
	 * @param sheet
	 */
	public static void createCellsCommonStyle(int fromRow,int fromCol,int toRow,int toCol,HSSFSheet sheet){
		HSSFCellStyle cellStyle = sheet.getWorkbook().createCellStyle();
		for(int i=fromRow;i<=toRow;i++){
			HSSFRow row = getRow(i, sheet);
			for(int j=fromCol;j<=toCol;j++){
		...
		if(cf.group){
			...
			for(Entry<String, List<Map<String, String>>> en : groupList){
				...
				for(Map<String, String> values : en.getValue()){
					ExcelUtil.createCellsCommonStyle(row, col, row, col+cf.fieldNames.size()-1, sheet);
